...
function Account() {
  ...
  const user= ()=>{
    axios.get("/api/v1/f/user/",{withCredentials: true}).then((res)=>{
      setData(res?.data[0]);
  ...
  return (
    <div className="w-screen h-screen px-5 pt-10 flex  flex-col an ">
      ...
      <div className="flex flex-1 flex-col items-center gap-4">
        ...
        <h1 className="r3 text-center">{data?.Name}</h1>
        <h1 className="r3 text-center">ID:{data?.Id}</h1>
        <h3 className="r2 text-center">{data?.Email}</h3>
    </div>
      ...
      <Link to="/My_order">
        <div className="w-full rounded-md bg-gray-100 h-10 flex justify-between items-center border border-gray-400 px-3">
          <span className=" items-center flex r3  gap-3"><CiHeart /> <h6 className="text-[0.6rem] ">My order</h6></span>
          <FaCaretRight />
      </div>
      </Link>
        <div className="w-full rounded-md bg-gray-100 h-10 flex justify-between items-center border border-gray-400 px-3">
        ...
        <div onClick={()=>logOut()} className="w-full rounded-md bg-gray-100 h-10 flex justify-between items-center border border-gray-400 px-3">
