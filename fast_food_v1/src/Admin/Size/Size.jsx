...
const fatch_user = ()=>{
  axios.get('/api/size',{withCredentials: true})
      .then(response => {
...
const deleteCategory = async (id) => {
  ...
  return (
    <div className="w-screen h-screen px-3 bg-blue-50 an">
      ...
      <h1 className="text-2xl r4 pb-3 px-3">Size</h1>
          ...
          <div className="p-1.5 min-w-full inline-block align-middle">
            <div className="border rounded-lg divide-y divide-gray-200 dark:border-neutral-700 dark:divide-neutral-700">
              <div className="py-3 px-4 bg-amber-100 flex gap-3.5">
               <Link to="/Admin/Size/Add_size">add</Link>
                    ...
                    {data.map((user, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                          ...
                          <Link to={`/Admin/Size/Edit_size/${user.Id}`} className="border-0 outline-0 text-yellow-600 px-1">Edit</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
