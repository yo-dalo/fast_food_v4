...
const fatch_Admin = ()=>{
  axios.get('http://localhost:3000/api/v1/ad/admin/',{withCredentials: true})
      .then(response => {
...
const deleteCategory = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/v1/ad/admin/${id}`);
      fatch_Admin(); // Refresh the list after deletion
    } catch (error) {
  ...
  return (
    <div className="w-screen h-screen px-3 bg-blue-50 an">
      ...
      <h1 className="text-2xl r4 pb-3 px-3">Admin</h1>
                ...
                <table className="min-w-full overflow-x-scroll divide-y divide-gray-200 dark:divide-neutral-700">
                  <thead className="bg-gray-50 dark:bg-neutral-700">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Token</th>
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                    {data.map((Admin, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">{Admin.Id}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{Admin.Name}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{Admin.Date}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{Admin.Email}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{Admin.Token}</td>
      
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
                          <button onClick={() => deleteCategory(Admin.Id)} className="border-0 outline-0 text-red-600 px-1">Delete</button>
                          <Link to={`/Admin/Admin/Edit_Admin/${Admin.Id}`} className="border-0 outline-0 text-yellow-600 px-1">Edit</Link>
                        </td>
                      </tr>
                    ))}
                  </tbody>
