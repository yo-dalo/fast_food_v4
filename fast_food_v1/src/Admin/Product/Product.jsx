...
function AdminProduct() {
  ...
  const fetchProducts = () => {
    axios.get("/api/v1/admin/Product/",{withCredentials: true})
      .then(response => {
  ...
  const changeStatus = async (id, status) => {
    try {
      await axios.patch(`/api/v1/admin/Product/${id}`, { status });
                ...
                <table className="min-w-full overflow-x-scroll divide-y divide-gray-200 dark:divide-neutral-700">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-neutral-700">
                    {filteredProducts.map((product, index) => (
                      ...
                      <Link to={`/Detail/${product.Id}`} > <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">{product.Id}</td></Link>
