...
function EditProduct() {
  ...
  const [size, setSize] = useState(null); // data Add_size  se aarha h 
  const [size_, setSize_] = useState(null); // data Add_size se aarha h only size_ array in fatch
...
console.log("______",size);
  ...
  const navigate = useNavigate();
    ...
    const fetchProduct = async (id) => {
      try {
        const response = await axios.get(`/api/Product/by_id/`+id);
        ...
        <div className="mt-12">
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 lg:gap-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                <div>
                  ...
                  >
                    ...
                    {categories.map((c, i) => (
                      <option key={i}  selected={product.Category_id==c.Id?true:false} value={c.Id}>{c.Id}</option>
                    ))}
                  </select>
                </div>
                <div>
              ...
              <Add_size edit={true} getdata={(data)=>setSize(data)} get_size={(data)=> setSize_(data)} />
              <Input/>
