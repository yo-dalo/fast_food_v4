...
const handle_add_to_card = async (i) => {
  ...
  try {
    const res = await axios.post("http://localhost:3000/api/card/", newCard,{withCredentials: true});
    //setProduct(res.data);
    
    setMy_card_items(my_card_items+1)
    alert(res.data.msg)
  } catch (err) {
    console.error(err);
  }
  ...
  const get_product = async (id) => {
    try {
      const res = await axios.get("http://localhost:3000/api/f/v2/Product/" + id,{withCredentials: true});
      setProduct(res.data);
        ...
        <div key={i.product_id} className="w-screen  h-screen px-5 sm:px-7 pt-10 flex flex-col relative an">
          <div className="flex h-10 justify-between items-center">
            ...
            <div className="flex text-2xl justify-center items-center gap-2">
              ...
              <Shoping_card_icon className={`text-black text-3xl rounded-md p-0.5`} _className="" /> 
          ...
          <div className="flex justify-between backdrop-blur-sm items-center sticky fixed bottom-2 z-10 left-0 duration-100">
            ...
            <button
              disabled={button}
              onClick={()=> login?handle_add_to_card(i):setLogIn_popUp(true)}
