...
const [user_address ,setUser_address] = useState("");
  ...
  try {
    const res = await axios.post("/api/f/v6/Card/", { my_card_arr },{withCredentials: true});
  ...
  const add_Order = async () => {
    if(!user_address||user_address==""||!my_card_arr.length){
      alert("place chack order")
    }else{
    try {
      const res = await axios.post("/api/v5/order",{
  ...
  return (
    <div className="w-screen h-screen px-5 pt-10 flex flex-col an relative  gap-3.5">
      <div className='flex w-full justify-between items-center'>
        <Button_1 />
        <h1 className="r3 text-xl">place order</h1>
        <div></div>
      </div>
      <div className="flex flex-1 flex-col items-center flex-1 gap-1.5 ">
        <div className="w-full flex flex-col gap-3">
          ...
          <div className="w-full flex flex-col gap-1">
            ...
            <div className="w-full rounded-md bg-gray-100 h-10 flex justify-between items-center border border-gray-400 px-3">
              <span className=" items-center flex r3  gap-3"><CiHeart /> <h6 className="text-[0.6rem]">Case on delevry</h6></span>
              <CiHeart />
            </div>
...
<div className="rounded-md bg-gray-100 border w-full  flex flex-col gap-1 px-3 py-4">
  ...
  <hr></hr>
  <div className=" w-full flex justify-between">
    <h2 className="text-1">Total</h2> <h2 className="text-1 r4">{total_rs}</h2>
  </div>
</div>
